    ...
    def __init__(self, data_x, data_y=None, batch_size=-1, num_batches=-1):
        ...
        if data_y is not None:
            if self.data_x.shape[0] != self.data_y.shape[0]:
                raise ValueError('data_x, data_y provided have different '
                                 'number of rows.')
...
class BatchSequenceInjector:
    ...
    Attributes:
        ...
        data_y (:obj:`numpy.ndarray`): Reference to input label array.s
    """
    def __init__(self, data_x, data_y=None, length=100, batch_size=-1,
                 num_batches=-1, with_seq=False):
    ...
    def next_batch(self, skip=1):
        ...
        if self.cur_batch == self.num_batches - 1:
            ...
            self.cur_batch += 1
        return self.to_sequence(self.length, self.data_x, self.data_y,
                                start, end, with_seq=self.with_seq)
    ...
    def to_sequence(length, x, y=None, start=None, end=None, with_seq=False):
        """Turn feature array as a sequence array where each new feature
        contains seq_len number of original features.

        Args:
            length (:obj:`int`): Length of the sequence.
            x (:obj:`numpy.ndarray`): Feature array, with shape
                (num_samples, num_features).
            y (:obj:`numpy.ndarray`): Label array, with shape
        ...
        Returns:
            ...
            seq_x is a numpy array of shape (num_samples, seq_len, num_features)
            and seq_y is a numpy array of shape (num_samples, num_classes).
            num_samples is bounded by the value of start and end.
            If start or end are not specified, the code will use the full data
            provided, so that the array returned has (num_samples - seq_len)
            of samples.
        """
...
class SkipGramInjector:
    ...
    def next_batch(self):
        ...
        self.data_index = (self.data_index + len(self.data_x) - span) % \
            len(self.data_x)
        return batch, labels
