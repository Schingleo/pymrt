...
def sensor2vec(num_sensors, sensor_sequence, embedding_size=20,
               ...
               num_neg_samples=64, learning_rate=1.0, num_steps=100001):
    """Sensor to Vector
    """
    if num_neg_samples > num_sensors:
        num_neg_samples = num_sensors
    # Initialize a SkipGram Injector
    injector = SkipGramInjector(sensor_sequence, batch_size, num_skips,
                                skip_window)
    ...
    with graph.as_default():
        ...
        # As we normally do not have too many sensors -
        # it is OK to use all of them
        valid_dataset = tf.constant([i for i in range(num_sensors)],
                                    dtype=tf.int32)
        ...
        with tf.Session(graph=graph) as session:
            ...
            distance_matrix = final_similarity / \
                np.max(final_similarity, axis=1)[:, None]
...
def sensor2vec_data(sensor_list, event_list, embedding_size=20,
    ...
    Similar to word embedding used in natural language processing system,
    we want to represent sensors using in a synthesized vector space as well,
    instead of using an arbitrary labels for each sensors without any useful
    information.

    The methods used to find word embeddings can be classified into two
    categories: count-based methods (Latent Semantic Analysis) and
    predictive models.
    In this implementation for mapping sensor into high dimension vector
    space, we use skip-gram negative sampling models.
    ...
    Args:
        ...
        num_skips (:obj:`int`): How many times to re-use an input to generate
            a label in skip-gram model.
        skip_window (:obj:`int`): How many items to consider left or right in
            skip-gram model.
        num_neg_samples (:obj:`int`): Number of negative samples to draw from
            the vocabulary.
        ignore_off (:obj:`bool`): Ignore motion-sensor with ``Off`` state in
            event.rst list.
        learning_rate (:obj:`float`): Learning rate for each step.
    ...
    # Initialize a SkipGram Injector
    injector = SkipGramInjector(event_sensor_list, batch_size,
    ...
    with graph.as_default():
        ...
        train_inputs = tf.placeholder(tf.int32, shape=[batch_size])
        train_labels = tf.placeholder(tf.int32, shape=[batch_size, 1])
        # As we normally do not have too many sensors - it is OK to use all
        # of them
        valid_dataset = tf.constant([i for i in range(num_sensors)],
                                    dtype=tf.int32)
        # Only CPU supports NCE loss
        ...
        # Compute the cosine similarity between minibatch examples and
        # all embeddings.
        norm = tf.sqrt(tf.reduce_sum(tf.square(embeddings), 1, keep_dims=True))
        normalized_embeddings = embeddings / norm
        valid_embeddings = tf.nn.embedding_lookup(
