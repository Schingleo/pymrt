...
from mtt2dcvid_bd_init import *


# Check point
chkpt_dir = '../../result/test/phd/2dcvid_sd'
...
def load_2dcv_states(model):
    states_chk_point = os.path.join(chkpt_dir, "2dcv_syn_noise_states.pkl")
...
def load_2dcv_observation(model, truth):
    observation_chk_point = os.path.join(chkpt_dir,
                                         "2dcv_syn_noise_observation.pkl")
...
if __name__ == "__main__":
    ...
    # GMPHD 3D plot grid
    grid_x, grid_y = np.mgrid[min_x:max_x:10, min_y:max_y:10]
    # Current step
    gmphd_step = 0
    # Total steps
