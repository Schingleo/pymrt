...
from pymrt.tracking.models import GmPhdCvIdModel, CubicUniformPoissonClutter
...
from pymrt.tracking.gmphd import gmphd_predictor, gmphd_corrector, \
    gm_pruning, gm_id_estimator
from pymrt.tracking.utils import gm_calculate
from mtt3dcvid_bd_init import *
...
def generate_3dcv_model():
    # Create 2D CV Model
    model = GmPhdCvIdModel(n=n)
    ...
    model._cz = 1/(2000*2000*2000)
    # Birth
    model.birth = model_birth
    return model


def generate_3dcv_states(model):
...
def load_3dcv_states(model):
    states_chk_point = os.path.join(chkpt_dir, "3dcv_syn_states.pkl")
    if os.path.exists(states_chk_point):
        chk_point_file = open(states_chk_point, 'rb')
        states = pickle.load(chk_point_file)
    else:
        states = generate_3dcv_states(model=model)
...
def generate_3dcv_observation(model, truth):
...
def load_3dcv_observation(model, truth):
    observation_chk_point = os.path.join(chkpt_dir, "3dcv_syn_observation.pkl")
    if os.path.exists(observation_chk_point):
        ...
        observation = generate_3dcv_observation(model=model, truth=truth)
...
if __name__ == "__main__":
    ...
    gmphd_model = generate_3dcv_model()
    gmphd_states = load_3dcv_states(gmphd_model)
    gmphd_observation = load_3dcv_observation(model=gmphd_model,
                                              truth=gmphd_states)
    #
    max_x = np.max([
    ...
    max_z = np.max([
        np.max([x[2, 0] for x in gmphd_observation[j]])
        for j in range(len(gmphd_observation))
    ])
    min_z = np.min([
        np.min([x[2, 0] for x in gmphd_observation[j]])
        for j in range(len(gmphd_observation))
    ])
    # GMPHD 3D plot grid
    grid_x, grid_y, grid_z = np.mgrid[
        min_x:max_x:50, min_y:max_y:50, min_z:max_z:50
    ]
    # Current step
    gmphd_step = 0
    # Total steps
    ...
    for i in range(gmphd_num_steps):
        ...
        # Get Prediction
        gmphd_prediction.append(gm_id_estimator(gm_list))
